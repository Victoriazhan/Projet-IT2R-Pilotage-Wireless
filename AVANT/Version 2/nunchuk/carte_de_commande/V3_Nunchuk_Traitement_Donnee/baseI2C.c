...
//	code à activer pour recevoir les différentes valeurs des autres capteurs
//	Driver_I2C0.MasterReceive (SLAVE_I2C_ADDR, &accx, 1, false); // récupération de la valeur de accélérometre x
//	while (Driver_I2C0.GetStatus().busy == 1);	// attente fin transmission
//	
//	Driver_I2C0.MasterReceive (SLAVE_I2C_ADDR, &accy, 1, false); // récupération de la valeur de accélérometre y
//	while (Driver_I2C0.GetStatus().busy == 1);	// attente fin transmission
//	
//	Driver_I2C0.MasterReceive (SLAVE_I2C_ADDR, &accz, 1, false); // récupération de la valeur de accélérometre z
//	while (Driver_I2C0.GetStatus().busy == 1);	// attente fin transmission
//	
	...
	
{
	...
	if ( (pos_X>=1) && (pos_X<=30) ) valeur_converti_pos_x = 0; //gauche extrème
	...
	else if ( (pos_X>230) && (pos_X<=254) ) valeur_converti_pos_x = 4; //droite extrème
